...
- Whatever attachments the framework captured. Playwright trace,
  Playwright/WDIO video, Jest snapshots, and so on.
...
Multiple test workers, multiple shards, multiple CI jobs all need to
...
Each one is short. A copy-pasteable config block, plus the
